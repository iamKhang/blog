                  ...
                  const handleKeyDown = async (e: React.KeyboardEvent<HTMLInputElement>) => {
                    if (e.key === 'Enter' && categorySearchValue.trim()) {
                      ...
                      const existingCategory = categories.find(
                        (category: Category) => category.name.toLowerCase() === categorySearchValue.toLowerCase()
                      );
                      ...
                      if (existingCategory) {
                        ...
                        try {
                          await createCategoryMutation.mutateAsync(categorySearchValue);
                          queryClient.invalidateQueries({ queryKey: ["categories"] });
                          toast({
                            title: "Success",
                  ...
                  const handleKeyDown = async (e: React.KeyboardEvent<HTMLInputElement>) => {
                    if (e.key === 'Enter' && tagSearchValue.trim()) {
                      ...
                      const existingTag = tags.find(
                        (tag: Tag) => tag.name.toLowerCase() === tagSearchValue.toLowerCase()
                      );
